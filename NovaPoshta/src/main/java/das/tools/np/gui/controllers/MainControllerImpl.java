...
public class MainControllerImpl implements MainController, Localized {
    ...
    public void updateNumber() {
        if (viewModeService.getCurrentMode() == ViewMode.VM_NUMBERS) {
            lvNumbers.getSelectionModel().getSelectedItem();
            new RunInThread(() -> numberService.updateNumberData(lvNumbers.getSelectionModel().getSelectedItem(), lvNumbers.getItems(), txLog)).run();
            ...
            if (number != null) {
                new RunInThread(() -> numberService.updateNumberData(number, tvGroups.getSelectionModel().getSelectedItem(), txLog)).run();
            }
            ...
            SimpleNumber number = conversionService.convert(getActiveSelectedNumber(), SimpleNumber.class);
            if (number != null) {
                new RunInThread(() -> numberService.updateNumberData(number, tvCalendar.getSelectionModel().getSelectedItem(), txLog)).run();
            }
